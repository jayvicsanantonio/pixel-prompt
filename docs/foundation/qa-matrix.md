...
| Gameplay telemetry is emitted once per real transition | Agent 3, Agent 5 | Happy-path and failure-path gameplay analytics are emitted without duplicate idempotent retries | Automated | `tests/unit/server/game-analytics.test.ts`, `tests/unit/server/game-http-analytics.test.ts` |
...
| Player refreshes during generation | Agent 3, Agent 4 | Progress is not corrupted; the attempt resolves to a recovered pending state or a refunded attempt | Partial | Interrupted provider failures already refund attempts in `tests/unit/server/game-http.test.ts` and `tests/unit/server/game-session-state.test.ts` | Run a real browser refresh during an in-flight request once the live submission path exists |
...
| Network interruption during submission | Agent 2, Agent 3 | Draft and progress survive the interruption, and duplicate retries do not double-consume attempts | Partial | Idempotency is covered in `tests/unit/server/game-http.test.ts`; local draft preservation is covered in `tests/unit/app/active-level-screen.test.tsx` | Validate browser offline/online behavior after the real form posts to the backend |
...
| Stored progress references a removed or unavailable level | Agent 3, Agent 5 | Resume falls back to the nearest valid current level and does not crash the run | Partial | Fallback logic lives in `src/server/game/session-state.ts` via `resolveResumeLevel`, but no explicit regression test exists yet | Add a targeted persistence or HTTP test before launch |
...
| Prompt is valid but intentionally nonsensical | Agent 4, Agent 5 | The prompt is still scored, attempts are consumed only for technically successful submissions, and tips remain reasonable | Partial | Off-topic successful prompts are covered in `tests/unit/server/game-http.test.ts`; low-quality scoring rules are documented in `docs/foundation/scoring-consistency-evaluation.md` | Add a manual pass using clearly nonsensical but policy-safe prompts against every seeded level |
...
| Service rate limits or provider timeouts | Agent 3, Agent 4 | Structured recoverable failure is shown, attempt is preserved, and failure analytics capture the provider category | Partial | Timeout behavior is covered in `tests/unit/server/game-http.test.ts` and `tests/unit/server/game-http-analytics.test.ts` | Add a dedicated rate-limit fixture or provider stub before launch |

## Launch-Blocking Gaps Still Open

- Browser-level refresh and offline interruption checks remain manual until Phase 2 connects the `/play` screen to the real backend.
- Generated-image display failure still needs a dedicated UI recovery path test.
- Removed-level resume recovery needs an explicit automated regression test.
- Rate-limit behavior is in the failure taxonomy but does not yet have a deterministic test fixture.
...
2. Execute manual browser QA for refresh-during-generation, offline retry, and broken generated-image display.
