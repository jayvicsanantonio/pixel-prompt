...
| Staging | `staging` branch | `DATABASE_URL`, `PIXEL_PROMPT_TARGET_ASSET_DIR`, `BLOB_READ_WRITE_TOKEN` | use non-production resources only; enable live OpenAI provider flags only when non-production quotas are ready |
| Production | `main` branch | `DATABASE_URL`, `PIXEL_PROMPT_TARGET_ASSET_DIR`, `BLOB_READ_WRITE_TOKEN` | production is the only environment allowed to use production analytics and provider credentials |
