...
describe("image scoring providers", () => {
  ...
  beforeEach(async () => {
    ...
    process.env.PIXEL_PROMPT_TARGET_ASSET_DIR = path.join(process.cwd(), ".tmp", `vitest-targets-${Date.now()}`);
    await rm(process.env.PIXEL_PROMPT_GENERATED_OUTPUT_DIR, { recursive: true, force: true });
    await rm(process.env.PIXEL_PROMPT_TARGET_ASSET_DIR, { recursive: true, force: true });
    await mkdir(path.join(process.env.PIXEL_PROMPT_TARGET_ASSET_DIR, "targets"), { recursive: true });
    await writeFile(path.join(process.env.PIXEL_PROMPT_TARGET_ASSET_DIR, "targets", "level-1.png"), Buffer.from(onePixelPngBase64, "base64"));
  });
  ...
  afterEach(async () => {
    ...
    if (process.env.PIXEL_PROMPT_GENERATED_OUTPUT_DIR) {
      await rm(process.env.PIXEL_PROMPT_GENERATED_OUTPUT_DIR, { recursive: true, force: true });
    }

    if (process.env.PIXEL_PROMPT_TARGET_ASSET_DIR) {
      await rm(process.env.PIXEL_PROMPT_TARGET_ASSET_DIR, { recursive: true, force: true });
    }
  });
